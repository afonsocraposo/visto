...
import {
  loginBackdropCacheKey,
  loginBackdropTTL,
  pickLoginBackdrop,
  readLoginBackdrop,
  saveLoginBackdrop,
  tmdbTitleURL,
} from "../src/features/auth/loginBackdrop.ts";

const media = (title, type, backdrop_path, tmdb_id = 123) => ({
  title,
  type,
  backdrop_path,
  tmdb_id,
...
test("Given trending media, When choosing a login backdrop, Then only media with wide art is eligible", () => {
  const selection = pickLoginBackdrop(
    {
      tv: [media("No art", "tv", ""), media("Show", "tv", "/show.jpg")],
      movies: [media("Film", "movie", "/film.jpg")],
    },
    1000,
    () => 0,
  );
...
test("Given cached trending media, When building its TMDB link, Then it uses the matching media type and ID", () => {
  ...
  assert.equal(
    tmdbTitleURL({ type: "movie", tmdb_id: 603 }),
