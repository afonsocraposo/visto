...
self.addEventListener("fetch", (event) => {
  ...
  if (event.request.method === "POST" && url.pathname === "/api/v1/auth/login") {
    event.respondWith(
      fetch(event.request).then(async (response) => {
        if (response.ok) await storeActiveUser(response.clone());
        return response;
      }),
    );
    return;
  ...
  if (event.request.method === "POST" && url.pathname === "/api/v1/auth/logout") {
    event.respondWith(
      fetch(event.request).then(async (response) => {
        if (response.ok) await clearActiveUserData();
        return response;
      }),
    );
    return;
      ...
      fetch(event.request)
        ...
        .catch(async () => {
          ...
          const cachedResponse = await (
            await caches.open(userCacheName(userID))
          ).match(event.request);
...
async function activeUserID() {
  const response = await (await caches.open(userIndex)).match(activeUserRequest);
  return response ? response.text() : null;
}
async function storeActiveUserID(userID) {
  await (await caches.open(userIndex)).put(activeUserRequest, new Response(userID));
}
async function cacheRecentResponse(userID, request, response) {
  ...
  for (const staleRequest of requests.slice(
    0,
    Math.max(0, requests.length - maxCachedAPIEntries),
  )) {
    await cache.delete(staleRequest);
  }
}
async function clearActiveUserID() {
  await (await caches.open(userIndex)).delete(activeUserRequest);
}
...
async function storeActiveUser(response) {
  try {
