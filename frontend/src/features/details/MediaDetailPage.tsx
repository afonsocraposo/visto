        ...
        return <Paper key={entry.episode.id} className="episode-row" withBorder p={0} role="button" tabIndex={0} onClick={openEpisode} onKeyDown={event => { if ((event.key === "Enter" || event.key === " ") && event.target === event.currentTarget) { event.preventDefault(); openEpisode(); } }}>
          <Group className="episode-row-layout" justify="space-between" wrap="nowrap" gap={0}>
            ...
            <Tooltip label={entry.watched ? "Already watched" : `Mark episode ${entry.episode.episode_number} watched`} withArrow>
              <ActionIcon className="episode-row-action" aria-label={entry.watched ? `Episode ${entry.episode.episode_number} already watched` : `Mark episode ${entry.episode.episode_number} watched`} size="lg" variant={entry.watched ? "light" : "default"} color={entry.watched ? "teal" : undefined} disabled={entry.watched || prepareEpisodeWatch.isPending || markEpisodeWatched.isPending} loading={!entry.watched && prepareEpisodeWatch.isPending} onClick={event => { event.stopPropagation(); requestEpisodeWatch(entry); }}>{entry.watched ? <IconEyeCheck size={18} /> : <IconEye size={18} />}</ActionIcon>
            </Tooltip>
          </Group>
        </Paper>;
      })}</Stack>
