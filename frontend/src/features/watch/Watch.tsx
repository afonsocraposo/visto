      ...
      <div className="watch-list">
        {entries.data.map(entry => {
          ...
          return <Paper key={entry.show_id} className={`watch-row${isCompleted ? " watch-row-completed" : ""}`} withBorder p={0} role={onOpenDetail && !isCompleted ? "button" : undefined} tabIndex={onOpenDetail && !isCompleted ? 0 : undefined} onAnimationEnd={event => { if (event.target === event.currentTarget) finishWatchedAnimation(entry.show_id); }} onClick={isCompleted ? undefined : openEpisode} onKeyDown={event => { if (!isCompleted && (event.key === "Enter" || event.key === " ") && event.target === event.currentTarget) { event.preventDefault(); openEpisode(); } }}>
            ...
            {isCompleted ? <ActionIcon className="watch-row-complete-indicator" size="xl" radius="xl" variant="light" color="teal" aria-label={`${entry.title} episode marked watched`}><IconCheck size={21} stroke={2.2} /></ActionIcon> : entry.next_episode && <Tooltip label="Mark episode watched" withArrow><ActionIcon className="watch-row-action" size="xl" radius="xl" variant="light" color="gray" aria-label={`Mark ${entry.title} season ${entry.next_episode.season_number}, episode ${entry.next_episode.episode_number} watched`} loading={markWatched.isPending} onClick={event => { event.stopPropagation(); entry.missing_prior_episodes?.length ? setConfirmation(entry) : markWatched.mutate({ episodeIDs: [entry.next_episode!.id], bulk: false, showID: entry.show_id }); }}><IconEye size={22} stroke={1.8} /></ActionIcon></Tooltip>}
          </Paper>;
        })}
      </div>
    </>
