...
export function AdminPanel({ currentUser }: { currentUser: User }) {
  ...
  const createUser = useMutation({
    mutationFn: async () => {
      ...
      if (!response.ok)
        throw new Error(
          (await response.json().catch(() => ({}))).error || "Could not create account.",
        );
      return response.json() as Promise<User>;
    },
    onSuccess: async () => {
    ...
    <Stack>
      <Paper withBorder p="md">
        ...
        <Text size="sm" c="dimmed" mt="xs">
          Create and maintain accounts for this Visto instance.
        </Text>
        {users.isError && (
      ...
      <Paper withBorder p="md">
        ...
        <form onSubmit={form.onSubmit(() => createUser.mutate())}>
          ...
          <Button type="submit" loading={createUser.isPending} mt="md">
            Create account
          </Button>
        </form>
      </Paper>
...
function AdminUserRow({ account, currentUser }: { account: User; currentUser: User }) {
  ...
  const update = useMutation({
    mutationFn: async () => {
      ...
      if (!response.ok)
        throw new Error(
          (await response.json().catch(() => ({}))).error || "Could not update account.",
        );
    },
    onSuccess: async () => {
      ...
      <Group justify="space-between" align="start">
        <div>
          ...
          <Text size="sm" c="dimmed">
            {account.email} · {account.role === "admin" ? "Admin" : "User"}
          </Text>
        </div>
        {account.id !== currentUser.id && (
          <Button
            ...
            onClick={() => {
              if (
                window.confirm(
                  `Delete ${account.name}'s account and personal data? This cannot be undone.`,
                )
              )
                remove.mutate();
            }}
