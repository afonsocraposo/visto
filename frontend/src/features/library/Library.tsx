...
export function LibraryPanel({ onOpenDetail, onOpenList }: { onOpenDetail?: (target: MediaDetailTarget) => void; onOpenList?: (status: LibraryStatus) => void }) {
  ...
  return (
    <>
      <div className="page-heading"><Text className="section-kicker">Your collection</Text><Title order={1}>Library</Title><Text c="dimmed" mt={6}>A quick view of everything you are tracking.</Text></div>
      ...
      <div className="library-sections">
        {sections.map(section => {
          ...
          return <section className="library-section" key={section.status} aria-labelledby={`library-section-${section.status}`}>
            <Group className="library-section-heading" justify="space-between" align="baseline" gap="sm">
              <div><Title id={`library-section-${section.status}`} order={2}>{section.label}</Title><Text size="sm" c="dimmed">{entries.length} {entries.length === 1 ? "title" : "titles"}</Text></div>
              {entries.length > PREVIEW_LIMIT && <Button variant="subtle" size="sm" onClick={() => onOpenList?.(section.status)}>Show all</Button>}
            </Group>
            <div className="library-grid">{visible.map(entry => <LibraryCard key={entry.item.media_id} entry={entry} onOpenDetail={onOpenDetail} />)}</div>
          </section>;
        })}
      </div>
    </>
