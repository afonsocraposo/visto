...
export function readLoginBackdrop(
  ...
  try {
    ...
    if (
      ...
      !Number.isSafeInteger(item.tmdb_id) ||
      item.tmdb_id! <= 0 ||
      !/^\/[a-zA-Z0-9_-]+\.(jpg|jpeg|png|webp)$/.test(item.backdrop_path)
    )
      return null;
...
export function pickLoginBackdrop(
  trending: TrendingResponse,
  now: number,
  random: () => number = Math.random,
): LoginBackdrop | null {
  const candidates = [...trending.tv, ...trending.movies].filter(
    (item) => item.backdrop_path && Number.isSafeInteger(item.tmdb_id) && item.tmdb_id > 0,
  );
  ...
  return {
    ...
    expiresAt: now + loginBackdropTTL,
  };
}
...
export function saveLoginBackdrop(
  storage: Pick<Storage, "setItem">,
  selection: LoginBackdrop,
): void {
  try {
