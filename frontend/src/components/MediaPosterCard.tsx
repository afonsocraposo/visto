...
export function MediaPosterCard({ media, onOpenDetail, target }: Props) {
  ...
  return <Paper className="media-poster-card" component="button" type="button" withBorder onClick={() => onOpenDetail?.(target ?? { mediaType: media.type, tmdbID: media.tmdb_id, seed: media })} aria-label={`Open details for ${media.title}`}>
    {art ? <Image src={art} alt="" /> : <div className="artwork-fallback">{media.title.slice(0, 1)}</div>}
