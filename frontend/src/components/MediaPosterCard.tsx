...
export function MediaPosterCard({ media, onOpenDetail, target, progress, subtitle }: Props) {
  ...
  const open = () =>
    onOpenDetail?.(target ?? { mediaType: media.type, tmdbID: media.tmdb_id, seed: media });
  return (
    <Paper
      ...
      onClick={open}
      onKeyDown={(event) => {
        if (onOpenDetail && (event.key === "Enter" || event.key === " ")) open();
      }}
      aria-label={onOpenDetail ? `Open details for ${media.title}` : undefined}
    >
      {!art && <div className="artwork-fallback">{media.title.slice(0, 1)}</div>}
      <span className="media-poster-card-scrim" aria-hidden="true" />
      <span className="media-poster-card-title">
        <Text component="span" fw={750} lineClamp={2}>
          {media.title}
        </Text>
