...
type HistoryEntry = { play: { id:string; media_id:string|null; episode_id:string|null; watched_at:string }; title:string; episode_label?:string };
...
function LibraryCard({entry}:{entry:LibraryEntry}) {
  ...
  const watched=useMutation({mutationFn:async()=>{const response=await fetch("/api/v1/plays",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({media_id:entry.item.media_id})});if(!response.ok)throw new Error("Could not record this watch.")},onSuccess:()=>queryClient.invalidateQueries({queryKey:["history"]})});
  ...
  return <Paper withBorder p="md" mt="sm"><Group justify="space-between"><Text fw={700}>{entry.media.title}</Text><Text size="sm" c="dimmed">{entry.media.type==="tv"?"TV show":"Movie"}</Text></Group><Group mt="sm" grow><Select aria-label={`Status for ${entry.media.title}`} value={entry.item.status} onChange={setStatus} data={[{value:"watchlist",label:"Watchlist"},{value:"watching",label:"Watching"},{value:"paused",label:"Paused"},{value:"dropped",label:"Dropped"}]} /><Select aria-label={`Rating for ${entry.media.title}`} value={entry.item.rating?String(entry.item.rating):"none"} onChange={setRating} data={[{value:"none",label:"Not rated"},...[1,2,3,4,5].map(value=>({value:String(value),label:`${value} / 5 stars`}))]} /></Group>{entry.media.type==="movie"&&<Button mt="sm" size="xs" loading={watched.isPending} onClick={()=>watched.mutate()}>Watched</Button>}{update.isError&&<Alert color="red" mt="sm">{update.error.message}</Alert>}{watched.isError&&<Alert color="red" mt="sm">{watched.error.message}</Alert>}</Paper>;
}

function HistoryPanel(){
  const history=useQuery({queryKey:["history"],queryFn:async()=>{const response=await fetch("/api/v1/plays?limit=100");if(!response.ok)throw new Error();return response.json() as Promise<HistoryEntry[]>}});
  ...
  return <Paper withBorder p="md" mt="lg"><Title order={2}>Watch history</Title>{history.data.length===0?<Text c="dimmed" mt="sm">No watches recorded yet.</Text>:history.data.map(entry=><HistoryCard key={entry.play.id} entry={entry} />)}</Paper>;
}

function HistoryCard({entry}:{entry:HistoryEntry}){
  const queryClient=useQueryClient();const [watchedAt,setWatchedAt]=useState(()=>{const date=new Date(entry.play.watched_at);return new Date(date.getTime()-date.getTimezoneOffset()*60000).toISOString().slice(0,16)});
  const save=useMutation({mutationFn:async()=>{const response=await fetch(`/api/v1/plays/${encodeURIComponent(entry.play.id)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({watched_at:new Date(watchedAt).toISOString()})});if(!response.ok)throw new Error("Could not correct this watch.")},onSuccess:()=>{void queryClient.invalidateQueries({queryKey:["history"]});void queryClient.invalidateQueries({queryKey:["continue"]});void queryClient.invalidateQueries({queryKey:["calendar"]});void queryClient.invalidateQueries({queryKey:["feed"]})}});
  const remove=useMutation({mutationFn:async()=>{const response=await fetch(`/api/v1/plays/${encodeURIComponent(entry.play.id)}`,{method:"DELETE"});if(!response.ok)throw new Error("Could not delete this watch.")},onSuccess:()=>{void queryClient.invalidateQueries({queryKey:["history"]});void queryClient.invalidateQueries({queryKey:["continue"]});void queryClient.invalidateQueries({queryKey:["calendar"]});void queryClient.invalidateQueries({queryKey:["feed"]})}});
  const rewatch=useMutation({mutationFn:async()=>{const body=entry.play.media_id?{media_id:entry.play.media_id}:{episode_id:entry.play.episode_id};const response=await fetch("/api/v1/plays",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});if(!response.ok)throw new Error("Could not record the rewatch.")},onSuccess:()=>queryClient.invalidateQueries({queryKey:["history"]})});
  return <Group justify="space-between" align="end" mt="md" wrap="wrap"><div><Text fw={600}>{entry.title}{entry.episode_label?` · ${entry.episode_label}`:""}</Text><Text size="xs" c="dimmed">Watched {new Date(entry.play.watched_at).toLocaleString()}</Text></div><Group align="end" gap="xs"><TextInput type="datetime-local" aria-label={`Watched time for ${entry.title}`} value={watchedAt} onChange={event=>setWatchedAt(event.currentTarget.value)} w={210} /><Button size="xs" variant="default" disabled={!watchedAt} loading={save.isPending} onClick={()=>save.mutate()}>Correct</Button><Button size="xs" variant="light" loading={rewatch.isPending} onClick={()=>rewatch.mutate()}>Rewatch</Button><Button size="xs" color="red" variant="subtle" loading={remove.isPending} onClick={()=>{if(window.confirm("Delete this individual watch?"))remove.mutate()}}>Delete</Button></Group>{(save.isError||rewatch.isError||remove.isError)&&<Alert color="red" w="100%">{save.error?.message||rewatch.error?.message||remove.error?.message}</Alert>}</Group>;
}
