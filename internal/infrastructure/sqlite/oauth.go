...
func (store *Store) ExchangeAuthorizationCode(ctx context.Context, codeHash, clientID, redirectURI, challenge, resource, accessHash, refreshHash string, now, accessExpiry, refreshExpiry time.Time) (oauth.Identity, error) {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO oauth_access_tokens(token_hash,client_id,user_id,scope,resource,expires_at,created_at) VALUES(?,?,?,?,?,?,?)`, accessHash, identity.ClientID, identity.UserID, scopes, resource, stamp(accessExpiry), stamp(now)); err != nil {
		return oauth.Identity{}, fmt.Errorf("save OAuth access token: %w", err)
	}
	if containsString(identity.Scopes, oauth.OfflineScope) {
		if _, err := tx.ExecContext(ctx, `INSERT INTO oauth_refresh_tokens(token_hash,client_id,user_id,scope,resource,expires_at,created_at) VALUES(?,?,?,?,?,?,?)`, refreshHash, identity.ClientID, identity.UserID, scopes, resource, stamp(refreshExpiry), stamp(now)); err != nil {
...
func (store *Store) FindOAuthAccessToken(ctx context.Context, tokenHash, resource string, now time.Time) (oauth.Identity, error) {
	...
	if _, err := store.DB.ExecContext(ctx, `UPDATE oauth_access_tokens SET last_used_at=? WHERE token_hash=?`, stamp(now), tokenHash); err != nil {
		return oauth.Identity{}, fmt.Errorf("record OAuth token use: %w", err)
	}
	return identity, nil
}
